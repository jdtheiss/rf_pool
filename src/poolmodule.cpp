...
template<typename T, typename fn>
static PyObject* lp_rf_pool(PyObject* args, PyObject* kwargs, fn pool_fn)
{
    // get inputs
    PyArrayObject* array = NULL;
    PyArrayObject* mask_indices = NULL;
    ...
    array = (PyArrayObject*) PyArray_FROM_O(PyTuple_GetItem(args, 0));
    float p = 1;
    if (check_kwargs(kwargs, "p")) {
        p = PyFloat_AsDouble(PyDict_GetItemString(kwargs, "p"));
    }
    mask_indices = (PyArrayObject*) PyArray_FROM_O(PyDict_GetItemString(kwargs, "mask_indices"));
    if (check_kwargs(kwargs, "retain_shape")) {
        retain_shape = PyObject_IsTrue(PyDict_GetItemString(kwargs, "retain_shape"));
    ...
    for (size_t i=0; i < size_t(dims_a[0]); ++i) {
        ...
        for (size_t j=0; j < size_t(dims_m[0]); ++j) {
            if (retain_shape) {
                // call rf pooling function while retaining mask_indices shape
                pool_fn((T*) PyArray_GETPTR1(array, i), p, (T*) PyArray_GETPTR1(mask, j),
                        mask_size, (T*) PyArray_GETPTR1(mask_indices, j),
                        (T*) PyArray_GETPTR1(output, i*dims_m[0] + j),
                        (size_t*) PyArray_GETPTR1(indices, i*dims_m[0] + j), apply_mask);
            } else {
...
template<typename T, typename fn>
static PyObject* lp_kernel_pool(PyObject* args, PyObject* kwargs, fn pool_fn, bool subsample = true)
{
    // get inputs
    PyArrayObject* array = NULL;
    size_t kernel[2];
    ...
    #pragma omp parallel for
    for (size_t i=0; i < size_t(dims_a[0]); ++i) {
        pool_fn((T*) PyArray_GETPTR1(array, i), p, kernel, img_shape, stride, img_size,
                (T*) PyArray_GETPTR1(output, i), (size_t*) PyArray_GETPTR1(indices, i));
    }
    Py_DECREF(array);
    return Py_BuildValue("(NN)", (PyObject*) output, (PyObject*) indices);
...
static PyObject* avg_pool(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    PyObject* index_mask = Py_None;
    if (check_kwargs(kwargs, "mask_indices")) {
        output_tuple = rf_pool<T, rf_fn>(args, kwargs, pool<T>::rf_avg_pool);
        index_mask = PyTuple_GetItem(output_tuple, 1);
    }
    Py_INCREF(index_mask);
    // apply kernel pooling function
    ...
    if (check_kwargs(kwargs, "kernel_size")) {
        if (check_kwargs(kwargs, "mask_indices")) {
            PyObject* local_args = PyTuple_GetSlice(output_tuple, 0, 1);
            Py_DECREF(output_tuple);
            output_tuple = kernel_pool<T, kernel_fn>(local_args, kwargs, pool<T>::kernel_avg_pool);
            Py_DECREF(local_args);
        } else {
            output_tuple = kernel_pool<T, kernel_fn>(args, kwargs, pool<T>::kernel_avg_pool);
        }
        index_kernel = PyTuple_GetItem(output_tuple, 1);
    }
    Py_INCREF(index_kernel);
...
template<typename T, typename rf_fn, typename kernel_fn>
static PyObject* lp_pool(PyObject* self, PyObject* args, PyObject* kwargs)
{
    // apply rf pooling function
    PyObject* output_tuple = NULL;
    PyObject* index_mask = Py_None;
    if (check_kwargs(kwargs, "mask_indices")) {
        output_tuple = lp_rf_pool<T, rf_fn>(args, kwargs, pool<T>::rf_lp_pool);
        index_mask = PyTuple_GetItem(output_tuple, 1);
    }
    Py_INCREF(index_mask);
    // apply kernel pooling function
    ...
    if (check_kwargs(kwargs, "kernel_size")) {
        if (check_kwargs(kwargs, "mask_indices")) {
            PyObject* local_args = PyTuple_GetSlice(output_tuple, 0, 1);
            Py_DECREF(output_tuple);
            output_tuple = lp_kernel_pool<T, kernel_fn>(local_args, kwargs, pool<T>::kernel_lp_pool);
            Py_DECREF(local_args);
        } else {
            output_tuple = lp_kernel_pool<T, kernel_fn>(args, kwargs, pool<T>::kernel_lp_pool);
        }
        index_kernel = PyTuple_GetItem(output_tuple, 1);
    }
    Py_INCREF(index_kernel);
...
typedef void (rf_lp_fn)(const float*, float, const float*, size_t, const float*, float*, size_t*, bool);
typedef void (kernel_lp_fn)(const float*, float, size_t*, size_t*, size_t*, size_t, float*, size_t*);
...
static PyMethodDef pool_methods[] = {
    {"max_pool",
     ...
     "max_pool(array, mask_indices=None, kernel_size=None, stride=None)\n\n \
     ...
     stride : tuple or int, optional\n \
         stride used for kernel pooling [default : kernel_size]\n\n \
     Returns\n \
     -------\n \
    ...
    {"avg_pool",
     ...
     METH_VARARGS | METH_KEYWORDS,
     "avg_pool(array, mask_indices=None, kernel_size=None, stride=None)\n\n \
     Ouput average across each receptive field (if mask_indices given) at max index,\n \
     followed by max across kernel (if given).\n\n \
     Parameters\n \
     ----------\n \
     array : numpy.ndarray\n \
     ...
     output : numpy.ndarray\n \
         output array with shape (batch*channel, img_height/kernel[0], img_width/kernel[1])\n \
         with the average value within each receptive field maintained and\n \
         all other values set to zero (if mask_indices != None). If kernel is given\n \
         kernel pooling is subsequently applied to the output (or input)."},
    {"lp_pool",
     (PyCFunction) lp_pool<float, rf_lp_fn, kernel_lp_fn>,
     METH_VARARGS | METH_KEYWORDS,
     "lp_pool(array, p=1., mask_indices=None, kernel_size=None, stride=None)\n\n \
     Ouput `sum([x_i**p for x_i in x])**(1/p)` across each receptive field\n \
     (if mask_indices given) set at max index, followed by max across kernel (if given).\n\n \
     Parameters\n \
     ----------\n \
     array : numpy.ndarray\n \
         input array with shape (batch*channel, img_height, img_width)\n \
     p : float\n \
         power to which each element is raised (e.g., `p=1` returns sum across RF)\n \
         [default : 1.] \n \
     mask_indices : numpy.ndarray or None\n \
         receptive field pooling indices with shape(n_RFs, img_height * img_width)\n \
     kernel_size : tuple or int, optional\n \
     ...
     output : numpy.ndarray\n \
         output array with shape (batch*channel, img_height/kernel[0], img_width/kernel[1])\n \
         with the LP output within each receptive field maintained and\n \
         all other values set to zero (if mask_indices != None). If kernel is given\n \
         kernel pooling is subsequently applied to the output (or input)."},
    {"probmax",
     (PyCFunction) probmax<float, rf_fn, kernel_fn>,
     METH_VARARGS | METH_KEYWORDS,
     "probmax(array, mask_indices=None, kernel_size=None, stride=None)\n\n \
    ...
    {"probmax_pool",
     ...
     METH_VARARGS | METH_KEYWORDS,
     "probmax_pool(array, mask_indices=None, kernel_size=None, stride=None)\n\n \
    ...
    {"stochastic_pool",
     ...
     METH_VARARGS | METH_KEYWORDS,
     "stochastic_pool(array, mask_indices=None, kernel_size=None, stride=None)\n\n \
    ...
    {"unpool",
     ...
     METH_VARARGS | METH_KEYWORDS,
     "unpool(array, mask=None, kernel_size=None, stride=None)\n\n \
